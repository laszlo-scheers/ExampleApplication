#!/usr/bin/env bash
# Just stopping the image to allow the CA to run
...
# Check if a docker container exists with the name of node_app and it removes the container
...
# Create a container called node_app that is available on port 8448 from our docker image
docker create -p 8448:8448 --name $CONTAINER_NAME $IMAGE_NAME
